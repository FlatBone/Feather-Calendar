...
- **ユーザー操作 → Update Logic:**
  ...
  - `app.rs` 内の `update` メソッドでこれらのイベントを処理し、`AppState` を変更する。
    ...
    - 例：曜日表記・週番号設定変更 → 次回の描画へ即時反映し、終了時にexe同階層のJSONへ保存する。
...
**永続化データスキーマ:**
exeと同じディレクトリの`feather_calendar_config.json`へ次のJSONデータを保存する。v0.1.5の既存4項目を維持し、`display_settings`を既定値付きで追加する。項目が欠落している場合は`Default`で補完し、読み込み不能なデータは設定全体を既定値へ戻す。
```json
{
  "window_position": { "x": 120.0, "y": 80.0 },
...
- **ユニットテスト (`cargo test`):**
  ...
  - **内容:**
    ...
    - v0.1.5形式のJSONから既存状態を維持したまま表示設定の既定値を補完できること。
...
- **E2E（エンドツーエンド）テスト/手動テスト:**
  - **内容:**
    ...
    9.  1ヶ月表示で週番号を表示/非表示にしても、右端が見切れず左右の余白が保たれることを確認。
    10. Windows 10/11の両環境で上記テストを実施する。
...
- **シリアライズ/デシリアライズ: `serde` / `serde_json`**
  - **理由:** exe同階層のポータブルな設定ファイルを使い、v0.1.5のJSON形式との後方互換性を維持する。

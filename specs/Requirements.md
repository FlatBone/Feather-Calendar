...
| **FR-03** | **ユーザーとして、** 曜日表記と週の開始曜日を用途に合わせて選びたい。 | - 曜日表記を英語（Sun., Mon., Tue., Wed., Thu., Fri., Sat.）または日本語（日、月、火、水、木、金、土）から選択できる。<br>- 週始まりを日曜日または月曜日から選択できる。<br>- 既定値は英語・日曜始まりである。<br>- 日曜は赤、土曜は青で表示され、列順を変更しても色が曜日に追従する。 |
| **FR-04** | **ユーザーとして、** 特定の日付を目立たせるために色をつけたい。 | - カレンダー上の任意の日付をクリックすると、その日の背景色がハイライトされる。<br>- 前月・翌月から補完表示された日付も同様に操作でき、同じ日付が複数の月枠に表示される場合は状態が同期する。<br>- 再度同じ日付をクリックすると、ハイライトが解除される。<br>- 複数の日付に色をつけることができる。 |
...
| **FR-06** | **ユーザーとして、** ウィンドウのサイズを変更したとき、カレンダーも追従して見やすく表示されてほしい。 | - ウィンドウの幅や高さを変更すると、3つのカレンダーのサイズとレイアウトが自動的に調整される。<br>- 1ヶ月表示では週番号の表示/非表示に応じてカレンダー幅を調整し、左右に余白を確保して見切れを防ぐ。<br>- ウィンドウを極端に小さくしても、表示が崩れない（例：縦一列に並ぶなど）。 |
...
| **FR-12** | **ユーザーとして、** 必要なときだけ週番号を確認したい。 | - 週番号列を表示または非表示にできる。<br>- 既定値は非表示である。<br>- ISO 8601方式では各行の木曜日が属するISO週番号を表示する。<br>- 日曜基準方式では各行の日曜日の日付が属するISO週番号を表示する。<br>- 週番号表示、週始まり、週番号基準は互いに独立して変更できる。 |
| **FR-13** | **ユーザーとして、** 曜日と週に関する表示設定を次回起動時も使いたい。 | - 曜日表記、週番号表示、週始まり、週番号基準がexeと同じディレクトリの`feather_calendar_config.json`へ保存される。<br>- v0.1.5の設定ファイルを読み込み、既存のウィンドウ位置、色付け情報、ピン留め、表示モードを維持できる。<br>- 保存済み設定は次回起動時に復元される。<br>- 設定が存在しない、項目が不足している、または破損している場合は安全に既定値へ戻る。 |
...
| **NFR-04** | **セキュリティ** | - アプリケーションは外部ネットワークへの通信を一切行わない。<br>- ユーザーの個人情報やPC内の他ファイルへのアクセスを要求しない。<br>- 設定と色付け情報はポータブル運用のためexeと同じディレクトリの単一JSONファイルに保存する。 |

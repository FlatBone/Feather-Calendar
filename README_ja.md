...
- 🗓️ **曜日・週表示のカスタマイズ**: 曜日を英語/日本語、週始まりを日曜/月曜から選択でき、ISO週番号も必要なときだけ表示できます。
...
#### 曜日・週番号の設定
ヘッダーの歯車ボタンから、次の設定を個別に変更できます。変更内容は自動保存され、次回起動時にも引き継がれます。

- **Weekday labels**: `EN`（Sun./Mon. …）または `日`（日/月 …）
- **Show week numbers**: カレンダー左端の週番号列を表示/非表示
- **Week starts**: `S`（日曜始まり）または `M`（月曜始まり）
- **Week-number rule**: `ISO` または `Sun`

`ISO` は各行の木曜日が属するISO 8601週番号を表示します。`Sun` は各行の日曜日の日付が属するISO週番号を表示します。週番号表示、週始まり、週番号基準は互いに独立しているため、日曜始まりのままISO週番号を表示することもできます。

設定はexeと同じディレクトリの`feather_calendar_config.json`へ保存されます。v0.1.5で作成された既存ファイルもそのまま読み込まれ、ウィンドウ位置、ハイライトした日付、ピン留め、表示モードを維持したまま新しい表示設定が追加されます。

#### 日付のハイライト
表示されたカレンダーの日付をクリックすると、前月・翌月から補完表示された日付も含めてハイライトできます。
...
日本語曜日表示には、SIL Open Font License 1.1で提供されるNoto Sans CJK JPのサブセットを使用しています。詳細は [フォントライセンス](assets/fonts/LICENSE.txt) を参照してください。
